...
No object can be cast to bottom, since bottom is an empty set.

An example of such a type is Kotlin's `Nothing`. `Nothing` is a type which cannot be constructed, which means it cannot be passed or returned either. It is considered an error for an instance of `Nothing` to exist. The way Kotlin prevents `Nothing` from being instantiated is [by setting its constructor to be private](https://github.com/JetBrains/kotlin/blob/7a7d392b3470b38d42f80c896b7270678d0f95c3/core/builtins/native/kotlin/Nothing.kt#L23).

The Java version of this type is [`Void`](https://docs.oracle.com/javase/8/docs/api/java/lang/Void.html), the wrapper class for the `void` primitive type. It is impossible to construct a `Void` instance because its constructor is private, and thus the only value a `Void` variable can hold is `null`. 
...
This type represents every value of every type there is; it represents the universal set of values, `U`. So `java.lang.Object`, basically. In Kotlin, this type is named `Any`. Meanwhile, `C` does as it do and overloads `void` by using `void *` to represent a pointer that can refer to a value of any type instead. How droll!
...
Java's `void` primitive technically works like this. When a method returns `void`, you can treat that method as if it implicitly returns the sole instance of the `void` type under the hood (Note that this is not how the JVM actually handles `void`). Java deviates from the theoretical norm in that `void` cannot ever be passed into a method as a parameter.

The truth is, Java just mashes the bottom and the unit types together to give us `void`.

You can technically simulate the unit type in Java by declaring a new class which is final and has no fields other than a static instance value. Then, you'd be able to treat that instance as the sole instance of the unit type.

In fact, this is exactly how Kotlin defines its unit type, `Unit`. If you navigate to the definition of `Unit`, you'll see how simply it is defined; [it's just an `object`](https://github.com/JetBrains/kotlin/blob/7a7d392b3470b38d42f80c896b7270678d0f95c3/core/builtins/src/kotlin/Unit.kt#L22)! 
...
In the struct above, `some_type` is a product type composed of four different types: `int`, `char *`, `double`, and `int` again. Notice that we're repeating `int` here. How do we figure out which `int` is which when we perform operations on `some_type`? Simple, we associate each type with the name given to it in the struct! Thus, we can say a product type is not merely just a list of types, but a list of *ordered pairs*, where each ordered pair consists of a type and a name associated with that type.
...
Now the example I just talked about only uses two component types `A` and `B`. How would we represent `some_type`, for example? The answer to that is to chain multiple product operations together, like so (I'm omitting the names of the fields here):
...
When Java 16 was released, the record class feature was stabilised. Record classes are a great example of what a product type is. All fields are final, and you can't inherit from these classes either. All of a record's state is set at the time of its construction[^mutablerec] and once a record is created, that's how it'll look for the rest of its lifetime, all 200 milliseconds of it. 

[^mutablerec]: Provided you don't have a mutable data type within your record.

This is in contrast to normal Java classes, which are all over the place. You can have public and private state, there's hidden state via inheritance that you don't think about until it pops up like some haunted animatronic on caffeine to jumpscare you with weird bugs, and you can have mutable fields, static fields, and all sorts of other distracting things that yadda yadda yadda... (*you get the idea*). 

The problem with normal Java types is that it is impossible to generalise what a type's components are. This becomes a problem when all you want is to efficiently process data; you have to navigate a maze of potentially non-standard getters to even get at your data in the first place, leave alone munge that data. 

Now, Java never did support destructuring like Typescript or Rust do. But even if Java had supported it, it is likely that the spec would still restrict that feature to records. Let's ask ourself a few questions to better understand why. 
...
That in turn means that the amazing people behind the Java Language Specification couldn't really figure out a way to allow pattern matching without also imposing non-backwards compatible restrictions on how classes worked.[^future]

[^future]: JEP-441 does mention this as [something to be taken up in the future](https://openjdk.org/jeps/441#Future-work). 

To fix this, they created records, an entirely different class hierarchy. There was already a precedent for this; Java 5 introduced enums, which inherit from `java.lang.Enum`. Similarly, all records inherit from `java.lang.Record`. 
...
- When you declare a record, and you do not define getter methods, getters will be defined using very specific syntax.
  - This syntax is very regular; Java just uses the name of the field as the name of the getter.
  - For field `a`, the getter would be `a()`.
  - If you manually define a getter that matches the naming conventions, Java will use your definition. Otherwise, Java will automatically create a getter method which correctly follows the conventions. The nonstandard getter won't make much of a difference.
...
These properties of records guarantee that any new language feature Java brings out that uses records such as pattern matching will always work, because the behaviour and structure of records in Java is guaranteed by the language spec itself.
...
You should be able to copy this code into a main method and run it by compiling with the flags `--enable-preview --release 20` and running with the flag `--enable-preview`, assuming your Java version is 20 (This is as of September 09, 2023).
...
Now, imagine a different problem. You want to have different colour representations such as RGB, HSL and CMYK. Maybe just make an enum for it?
...
Now obviously, this is NOT how anybody who knows Java would design the `Color` class. There's a much better way of implementing multiple colour representations without sacrificing readability: just use polymorphism! (I've put this in a collapsible section because it's quite long)
...
Now, given a `Color` instance, you'd just need to check if it is an `instanceof` your desired colour representation, and you'd be able to access data from that representation. But this implementation has a flaw. How do we restrict what a colour is in our class hierarchy? For example, any user of your library could create a new `RYB` class that inherits from `Color`, or from `RGB`, for example. This becomes a problem when your library does not expect any new variants of `Color` to exist, or if it does not expect certain `Color` variants to change their behaviour. Unless the API is designed to be extensible, creating new representations could in the best case cause crashes (So you have a chance of knowing what went wrong) or in the worst case, subtle bugs that end up affecting code far away from the problem's source.
...
Sum types encode the fact that a type can be *any one* of its constituents at a single point of time. They are also known as union types, because in type theory, they are usually represented as a type whose range of values is the union set of its components.
...
When you hear the term union types, you may be reminded of `C`'s `unions`. 
...
Note that the value of the union is overwritten in the second assignment of `doubleValue`. If you were to print `myUnion.intValue` after the second assignment, you'll see gibberish; that's actually just the bytes of `doubleValue` cut in half and interpreted as an integer.

But `C` unions have a flaw; there's no built-in way to know which variant is contained within a union value without external information. Thus, we need an external *discriminant* to figure out what's inside. Java's polymorphism can do that for us with `instanceof`. But Java's class hierarchy is too open; there's no way to restrict the number of variants of a Java "union".

What we need are *tagged unions*, which is exactly what sealed types allow us to represent. The reason the `sealed` modifier exists is to make it clear that you can't extend a sealed class beyond the classes that are allowed to inherit from it. This allows the developer to control how users interact with their library's API.
...
If your class hierarchy starts with a `sealed` class, you have to make sure to mark all inheritors (direct or indirect) as `sealed`, `non-sealed`, or `final`. If an inheriting class doesn't have these modifiers, it is a compile error.
...
- `non-sealed` - The class can be inherited normally. Useful for controlling the scope of custom behaviours.
...
Now, imagine we're writing some code to handle colours based on their format. We already have a nice, restricted way of ensuring there'll be no funny business. But how would we structure the code that handles the sealed class instances?
...
You may notice we aren't using a default case here. Normally, Java would have raised an error stating all cases haven't been covered. However, because `Color` is a sealed class, Java can tell that every case has been handled.

### Guards! Guards! 

Did I mention we've got guard clauses? Guard clauses! You can attach additional conditions to switch arms that have to be true for the arm to be executed! Guard clauses allow for succinct expression of more complex conditions within switch statements and expressions. No more extremely nested `if` conditions in your switches.
...
Java 21 allows us to integrate that condition within the case label itself with the `when` keyword.
...
{{< note-alert >}}
Java will still eagerly match whichever case evaluates to true first; so make sure to put more specific cases (guarded or not) first, followed by less specific cases.
{{< /note-alert >}}
...
The example here is very easy to spot; we can easily statically determine that there's a divide by zero error here. However, if the dividend were to be a dynamic, possibly `0` value, the waters would be muddier.
...
We've looked at a bunch of things that Java 21 allows us to do in this article (I haven't covered certain things like how generics interact with switch patterns, however). In the next one, I'll show you some interesting quirks, and a few practical examples of how we can leverage these functional building blocks to improve the way we write Java code.
